        ...
        return (
          <span
            ...
            style={{
              ...
              opacity: wordOpacity,
              // Word gap spacing
              marginRight: `${14 * scaleFactor}px`,
              marginBottom: `${10 * scaleFactor}px`,
            }}
          >
            {letters.map((letter, letterIndex) => {
              // Shine sweep effect: moves gradient background position depending on frames
              const shineSpeed = 50; // frames per sweep loop
              const shinePos = (frame % shineSpeed) * (100 / shineSpeed);
              const activeBackground = `linear-gradient(120deg, ${secondaryColor} 35%, #ffffff 50%, ${secondaryColor} 65%)`;
              const activeBackgroundSize = "200% 100%";
              const activeBackgroundPosition = `${100 - shinePos * 2}% 0`;

              const letterScale = isActive ? 1.15 : 1.0;
              ...
              return (
                <span
                  ...
                  style={{
                    ...
                    // Letter-level custom gap/spacing
                    marginRight: `${3 * scaleFactor}px`,
                    // Deep drop shadow
                    textShadow: `0 ${8 * scaleFactor}px ${16 * scaleFactor}px rgba(0,0,0,0.85)`,
                    transform: `scale(${letterScale})`,
                    transition: "transform 0.15s cubic-bezier(0.175, 0.885, 0.32, 1.1)",
                    // Shine implementation on active word letters
                    ...(isActive
                      ? {
